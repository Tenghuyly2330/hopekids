<x-app-layout>
    <div class="max-w-7xl mx-auto shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold text-[#401457]">Edit Program</h2>
        <form action="{{ route('program.update', $program->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="number_en" class="block text-sm font-medium text-[#000]">Number (English)</label>
                        <input value="{{ old('number_en', $program->number_en) }}" type="text" name="number_en" id="number_en"
                            class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">
                        <x-input-error class="mt-2" :messages="$errors->get('number_en')" />
                    </div>
                    <div>
                        <label for="title_en" class="block text-sm font-medium text-[#000]">Title (English)</label>
                        <input value="{{ old('title_en', $program->title_en) }}" name="title_en" id="title_en"
                    ...
                    <div>
                        ...
                        <textarea name="content_en" id="content_en" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]">{{ old('content_en', $program->content_en) }}</textarea>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="number_km" class="block text-sm font-medium text-[#000]">Number (Khmer)</label>
                        <input value="{{ old('number_km', $program->number_km) }}" type="text" name="number_km" id="number_km"
                            class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">
                        <x-input-error class="mt-2" :messages="$errors->get('number_km')" />
                    </div>
                    <div>
                        <label for="title_km" class="block text-sm font-medium text-[#000]">Title (Khmer)</label>
                        <input value="{{ old('title_km', $program->title_km) }}" name="title_km" id="title_km"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]"></input>
                    ...
                    <div>
                        ...
                        <textarea name="content_km" id="content_km" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]">{{ old('content_km', $program->content_km) }}</textarea>
            ...
            <div>
                ...
                <label for="dropzone-file{{ $program->id }}" id="drop-area"
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                        id="img-preview" style="background-image: url('{{ asset($program->image) }}')">
                    ...
                    <input id="dropzone-file{{ $program->id }}" type="file" class="hidden" name="image"
