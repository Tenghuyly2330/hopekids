<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        ...
        <form action="{{ route('event_backend.update', $event->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('title_en', $event->title_en) }}" type="text" name="title_en"
                    ...
                    <div>
                        ...
                        <textarea name="content_en" id="content_en" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]">{{ old('content_en', $event->content_en) }}</textarea>
                ...
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        <label for="title_km" class="block text-sm font-medium text-gray-700">Title</label>
                        <input value="{{ old('title_km', $event->title_km) }}" type="text" name="title_km"
                            id="title_km" class="mt-1 block w-full p-2 border rounded-md text-black text-sm">
                        <x-input-error class="mt-2" :messages="$errors->get('title_km')" />
                    </div>
                    <div>
                        <label for="content_km" class="block text-sm font-medium text-gray-700">Content</label>
                        <textarea name="content_km" id="content_km" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]">{{ old('content_km', $event->content_km) }}</textarea>
            ...
            <div>
                <label for="dropzone-file{{ $event->id }}" id="drop-area"
                    ...
                    <input id="dropzone-file{{ $event->id }}" type="file" class="hidden" name="images[]"
            ...
            <div class="flex justify-between">
                <a href="{{ route('event_backend.index') }}"
    ...
    <script>
        ...
        const fileInput = document.getElementById("dropzone-file{{ $event->id }}");
