<x-app-layout>
    <div class="max-w-7xl mx-auto shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold text-[#401457]">Edit Article</h2>
        <form action="{{ route('article_backend.update', $article->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <label for="dropzone-file{{ $article->id }}" id="drop-area"
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                        id="img-preview" style="background-image: url('{{ asset($article->image) }}')">
                    ...
                    <input id="dropzone-file{{ $article->id }}" type="file" class="hidden" name="image"
            ...
            <div class="flex justify-between">
                <a href="{{ route('article_backend.index') }}"
