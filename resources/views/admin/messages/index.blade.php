                    ...
                    @foreach ($messages as $message)
                        <tr class="border border-[#4FC9EE]">
                            ...
                            <td class="py-1 px-4 text-xs max-w-[200px] border-r border-[#4FC9EE]">
                                <img src="{{ asset($message->image) }}" alt="" class="w-12 h-12 object-cover object-top">
                            </td>
                            <td class="py-4 px-4 text-xs max-w-[200px] border-r border-[#4FC9EE]">
                                <div class="flex flex-col truncate">
                                    <p>{{ $message->name_en }}</p>
                                    <p>{{ $message->position_en }}</p>
                                </div>
                            </td>

                            <td class="py-4 px-4 text-xs max-w-[200px] border-r border-[#4FC9EE]">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('message.edit', $message->id) }}" title="Edit">
                                    ...
                                    {{-- <a href="#" title="Delete"
                                        onclick="event.preventDefault(); deleteRecord('{{ route('message.delete', $message->id) }}')">
